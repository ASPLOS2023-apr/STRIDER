    ...
    def dfNodeToAstNode(self, dstTerm, parameterinfo, dfNode, actType=None):
        ...
        if astNode:
            if self.spsAstNode == None and actType == "node":
                if parameterinfo == "nonblocking":
                    astNode = NonblockingSubstitution(Lvalue(Identifier(dstTerm)), Rvalue(astNode))
                elif parameterinfo == "blocking" or parameterinfo == None:
                    astNode = BlockingSubstitution(Lvalue(Identifier(dstTerm)), Rvalue(astNode))
            return astNode
        if isinstance(dfNode, DFBranch):
            ...
            if isinstance(dfNode.truenode, DFBranch):
                ...
                if parameterinfo == "nonblocking":
                    trueStatement = NonblockingSubstitution(Lvalue(Identifier(dstTerm)), Rvalue(self.dfNodeToAstNode(dstTerm, parameterinfo, dfNode.truenode)))
                elif parameterinfo == "blocking" or parameterinfo == None:
                    trueStatement = BlockingSubstitution(Lvalue(Identifier(dstTerm)), Rvalue(self.dfNodeToAstNode(dstTerm, parameterinfo, dfNode.truenode)))
            ...
            if isinstance(dfNode.falsenode, DFBranch):
                ...
                if parameterinfo == "nonblocking":
                    falseStatement = NonblockingSubstitution(Lvalue(Identifier(dstTerm)), Rvalue(self.dfNodeToAstNode(dstTerm, parameterinfo, dfNode.falsenode)))
                elif parameterinfo == "blocking" or parameterinfo == None:
                    falseStatement = BlockingSubstitution(Lvalue(Identifier(dstTerm)), Rvalue(self.dfNodeToAstNode(dstTerm, parameterinfo, dfNode.falsenode)))
                elif parameterinfo == "assign":
                    falseStatement = self.dfNodeToAstNode(dstTerm, parameterinfo, dfNode.falsenode)
